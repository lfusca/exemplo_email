  ...
  for email_check in new_email_ids:
    ...
    if your_email not in sender_email:
      ...
      if responder.lower() == "s":
        ...
        with open(current_path+'/email.txt', 'w', encoding='utf-8') as f:
            f.write(email_body)
          
        
        os.system("poetry run food_truck")

        print("Feito!")

        # load reply email_body from file email.txt
        with open('c:/Agentes/food_truck/resposta.txt', 'r', encoding='utf-8') as f:
            reply = f.read()
